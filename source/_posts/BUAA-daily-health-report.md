...
If the `python` script reports this error:

> 'ascii' codec can't encode characters in position 12-15: ordinal not in range(128)

It should be an encoding issue. Following [an online tutorial](https://www.jianshu.com/p/4e2000920332), add an environment variable before the command:
...
Together with Unix/Linux [crontab](https://linuxtools-rst.readthedocs.io/zh_CN/latest/tool/crontab.html), this can run the check-in automatically every day.

Contents of the `rentaocron` file:
...
## Getting the Check-In Address

### Configure a Proxy Server on Mac
